...
export default function MoveList({ game, currentPly, onSeek, blindMode }) {
    ...
    <div className="ml" ref={listRef}>
      {pairs.map((p) => (
        <div className="ml-row" key={p.n}>
          ...
          {p.white && (
            <MoveChip m={p.white} ply={plyFor(p.n, "w")}
              active={currentPly === plyFor(p.n, "w")}
              onClick={() => onSeek(plyFor(p.n, "w"))}
              blind={blindMode} youPlayed />
          )}
          {p.black && (
            <MoveChip m={p.black} ply={plyFor(p.n, "b")}
              active={currentPly === plyFor(p.n, "b")}
              onClick={() => onSeek(plyFor(p.n, "b"))}
              blind={blindMode} youPlayed={false} />
          )}
        </div>
      ))}
      <style>{`
