...
function Board({
  ...
  flipped = false,
  size = 560,
  lastMove = null,          // { from: "e4", to: "e5" } or just { to: ... }
  highlight = null,         // square like "f7"
  highlightKind = "critical", // "critical" | "best" | "danger" | "hint"
  arrow = null,             // { from, to, color }
  arrows = [],              // [{from,to,color}]
  showCoords = true,
  onSquareClick = null,
  selectedSquare = null,
  annotations = [],          // [{square, type}]
  ghostPiece = null,         // { square, piece } for "what if" preview
}) {
  const sqSize = size / 8;
  ...
  return (
    <div className="board-root" style={{ width: size, height: size }}>
      ...
      >
          ...
          Array.from({ length: 8 }).map((_, x) => {
            ...
            return (
              <g key={`sq-${x}-${y}`}>
                ...
                {showCoords && x === 0 && (
                  <text
                    x={x * sqSize + 4} y={y * sqSize + 13}
                    fontSize={Math.round(sqSize * 0.18)}
                ...
                {showCoords && y === 7 && (
                  <text
                    x={x * sqSize + sqSize - 10} y={y * sqSize + sqSize - 4}
                    fontSize={Math.round(sqSize * 0.18)}
                    fontFamily="var(--font-mono)"
                    fill={isLight ? "var(--square-dark)" : "var(--square-light)"}
                    opacity="0.55"
        ...
        {/* Custom square highlight (critical moment etc) */}
        {highlightPos && (
        ...
        {/* Ghost piece for variation preview */}
        {ghostPiece && (() => {
        ...
        {/* Annotations: small circles/squares */}
        {annotations.map((a, i) => {
          ...
          if (a.type === "circle") {
            return (
              <circle key={`ann-${i}`}
                cx={pos.cx} cy={pos.cy} r={sqSize * 0.42}
        ...
        {/* Arrows */}
        <defs>
          <marker id="arrow-accent" viewBox="0 0 10 10" refX="6" refY="5"
                  markerWidth="5" markerHeight="5" orient="auto-start-reverse">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="var(--accent)" />
          </marker>
          <marker id="arrow-info" viewBox="0 0 10 10" refX="6" refY="5"
                  markerWidth="5" markerHeight="5" orient="auto-start-reverse">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="var(--info)" />
          </marker>
          <marker id="arrow-danger" viewBox="0 0 10 10" refX="6" refY="5"
                  markerWidth="5" markerHeight="5" orient="auto-start-reverse">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="var(--danger)" />
          </marker>
          <marker id="arrow-warn" viewBox="0 0 10 10" refX="6" refY="5"
                  markerWidth="5" markerHeight="5" orient="auto-start-reverse">
...
// Eval bar (vertical)
function EvalBar({ evalCp, height = 560, flipped = false }) {
  // Clamp eval into [-800, 800] → [0, 1]
...
Object.assign(window, { Board, EvalBar });
