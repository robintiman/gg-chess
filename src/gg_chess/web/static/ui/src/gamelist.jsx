    ...
    <div className="gl-root">
      ...
      <style>{`
        ...
        .gl-title-sub {
          ...
          font-family: var(--font-serif); letter-spacing: 0.3px;
        }
        .gl-sync {
          background: none;
...
function GameCard({ game, active, onClick }) {
  ...
  const resultColor = game.result === "win" ? "var(--accent)"
                   ...
                   : "var(--text-dim)";
  const learning = Math.round(game.interest * 100);

  return (
    <button className={`gc ${active ? "active" : ""}`} onClick={onClick}>
      <div className="gc-top">
        <div className="gc-result" style={{ color: resultColor, borderColor: resultColor }}>
          {resultChar}
        </div>
        <div className="gc-info">
          <div className="gc-vs">
            <span className="gc-opp">vs. {game.opponent}</span>
            <span className="gc-rating">{game.oppRating}</span>
          </div>
      ...
      <div className="gc-bottom">
        <div className="gc-meta">
          <span className="gc-date">{shortDate(game.playedAt)}</span>
          <span className="gc-dot">·</span>
          <span className="gc-tc">{game.timeControl}</span>
          <span className="gc-dot">·</span>
...
Object.assign(window, { GameList, GameCard, LearningPip, shortDate });
