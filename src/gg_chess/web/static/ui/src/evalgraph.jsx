// Eval graph: shows the centipawn eval across all plies as an area chart,
// with critical-moment markers.

function EvalGraph({ plies, currentPly, onSeek, criticalPlies = [] }) {
  const [hoverPly, setHoverPly] = React.useState(null);
  ...
  const padX = 10;
  const padY = 12;
  const n = plies.length - 1; // skip starting position at idx 0
  ...
  const evalToY = (v) => {
    const c = clamp(v);
    const t = (c + 800) / 1600; // 0..1, 0 = black winning
    return padY + (1 - t) * innerH;
  };
  const plyToX = (i) => padX + (i / Math.max(1, n)) * innerW;

  // Build area paths for white / black advantage
  ...
  // Smooth line via cubic spline (simple — Catmull-Rom → cubic)
  ...
  const areaWhite = `${linePath} L ${pts[pts.length-1]?.x || padX} ${zeroY} L ${padX} ${zeroY} Z`;

  function handleMove(e) {
    const rect = ref.current.getBoundingClientRect();
    const x = (e.clientX - rect.left) * (width / rect.width);
    const i = Math.round(((x - padX) / innerW) * n);
    const ply = Math.max(1, Math.min(n, i));
    setHoverPly(ply);
  }

  function handleClick() {
    if (hoverPly != null) onSeek?.(hoverPly);
  }
  ...
  return (
    <div className="eval-graph">
      ...
      >
        ...
        {/* Upper band fill (White advantage) */}
        <defs>
...
Object.assign(window, { EvalGraph, formatMoveLabel, formatEval });
